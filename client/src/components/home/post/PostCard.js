...
function PostCard({ post, type }) {
  ...
  const addheart = async () => {
    try {
      const data = await likes(post._id, user.id);
      if (data.msg === "exists") {
        setheart(true);
  ...
  const removeheart = async () => {
    try {
      const data = await deletelikes(post._id, user.id);
      if (data.msg === "deleted") {
      } else {
  ...
  const setbookmark = async () => {
    try {
      const data = await bookmark(post._id, user.id);
      if (data.msg === "exists") {
  ...
  const dellbookmarl = async () => {
    try {
      const data = await deletebookmark(post._id, user.id);
      if (data.msg === "deleted") {
        setbook(false);
        setsbook(false);
      } else {
  ...
  return (
    <div className="item">
      ...
      <div className="right">
        <div className="title">
          <h3 onClick={navigateToArticle}>{post && post.title}</h3>
          {!type || type != "powner" ? (
            <>
        ...
        {type && type === "powner" ? (
          <>
            <div className=" profile_data">
              ...
              |
              <div className="red" onClick={() => deleteposts(post._id)}>
                Delete
              </div>
            </div>
            <RWebShare
                ...
                {post?.user && (
                  <Link
                    to={
                      !user
                        ? "/auth"
                        : `/ProfileRedirect/${post.user._id || -1}`
                    }
                  >
                    <img
                      className="imgscp"
              ...
              <div className="user_middle">
                {!user ? (
                  <span className="user_name">
                    <Link to={`/auth`}>N/A</Link>
                  </span>
                ) : (
                  <span className="user_name">
                    <Link
                      to={`/ProfileRedirect/${
                        post && post.user._id ? post.user._id : -1
                      }`}
                    >
                      {post && post.user.name}
                    </Link>
